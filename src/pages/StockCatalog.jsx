...
const SearchBar = ({ value, onChange, onClear, isSearching, placeholder }) => (
...
const WatchlistButton = ({ ticker, isWatched, isLoading, onAdd, onRemove, t }) => {
  if (isLoading) {
    return (
      <button
        disabled
  ...
  if (isWatched) {
    return (
      <button
        ...
        title={t('catalog.btn_remove_title')}
      >
        <StarOff className="w-3 h-3" />
        <span className="hidden sm:inline">{t('catalog.btn_remove')}</span>
      </button>
    );
  }
...
const Pagination = ({ currentPage, totalPages, onPageChange, isLoading, t }) => {
...
const StockCatalog = () => {
  ...
  const fetchStocks = useCallback(async (page, search) => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || err.message || t('catalog.no_results_empty');
      setStocksError(msg);
      setStocks([]);
    } finally {
  ...
  const handleAddToWatchlist = async (ticker) => {
    ...
    showToast(`${t('catalog.toast_adding')} ${ticker}…`);
    try {
  ...
  const handleRemoveFromWatchlist = async (ticker) => {
    setButtonLoading(prev => ({ ...prev, [ticker]: true }));
    showToast(`${t('catalog.toast_removing')} ${ticker}…`);
    try {
  ...
  if (isLoadingStocks && isLoadingWatchlist && stocks.length === 0 && !searchQuery) {
    return <PageLoader label={t('catalog.loading')} />;
  }
          ...
          <div>
            ...
            <p className="font-mono text-[10px] tracking-[2px] uppercase text-text-muted">
              {isSearchActive && !isLoadingStocks
                ? `${pagination.totalItems} ${t('catalog.results_for')} "${searchQuery}"`
                : pagination.totalItems > 0
                  ? `${pagination.totalItems.toLocaleString('id-ID')} ${t('catalog.stocks_available')}`
                  : isLoadingStocks ? t('catalog.loading_short') : '—'
              }
            </p>
          </div>
      ...
      {noResultsFound && !stocksError && (
        <div className="flex flex-col items-center justify-center min-h-[40vh] gap-4">
          ...
          <p className="font-mono text-[11px] tracking-[1px] uppercase text-text-muted">
            {isSearchActive ? `${t('catalog.no_results_search')} "${searchQuery}"` : t('catalog.no_results_empty')}
          </p>
          {isSearchActive && (
            <button
              onClick={handleSearchClear}
        ...
        <>
          <div className="hidden md:block bg-card-dark border border-card-border">
            <div className="grid grid-cols-[1.8fr_1fr_1fr_1.2fr] gap-4 px-5 py-3.5 border-b border-card-border">
              <span className="font-mono text-[9px] tracking-[1.5px] uppercase text-text-muted">{t('catalog.col_stock')}</span>
              <span className="font-mono text-[9px] tracking-[1.5px] uppercase text-text-muted text-right">{t('catalog.col_price')}</span>
              <span className="font-mono text-[9px] tracking-[1.5px] uppercase text-text-muted text-right">{t('catalog.col_change')}</span>
              <span className="font-mono text-[9px] tracking-[1.5px] uppercase text-text-muted text-right">{t('catalog.col_action')}</span>
            </div>
                        ...
                        <div className="flex items-center justify-end gap-2" onClick={e => e.stopPropagation()}>
                          {isWatched && (
                            <button
                              ...
                              title={t('catalog.btn_analyze_title')}
                            >
                              <BarChart2 className="w-3 h-3" />
                              <span className="hidden lg:inline">{t('catalog.btn_analyze')}</span>
                            </button>
                          )}
                          <WatchlistButton
                            ticker={ticker}
                  ...
                  return (
                    <div key={ticker} className="bg-card-dark border border-card-border p-4">
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-hairline">
                        <div>
                          <p className="font-mono text-[9px] tracking-[1.5px] uppercase text-text-muted mb-1">{t('catalog.col_price')}</p>
                          {stock.current_price != null ? (
                        ...
                        <div className="text-right">
                          <p className="font-mono text-[9px] tracking-[1.5px] uppercase text-text-muted mb-1">{t('catalog.col_change')}</p>
                          <ChangeIndicator value={stock.change_percent} />
                        </div>
                        {isWatched && (
                          <button
          ...
          {!isLoadingStocks && stocks.length > 0 && (
            <p className="text-center font-mono text-[10px] tracking-[1px] uppercase text-text-muted pb-4">
              {t('catalog.pagination_info')} {pagination.currentPage} {t('catalog.pagination_of')} {pagination.totalPages}
              {' · '}{pagination.totalItems.toLocaleString('id-ID')} {t('catalog.pagination_total')}
            </p>
          )}
        </>
      )}
